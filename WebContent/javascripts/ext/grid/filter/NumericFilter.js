...
Ext.grid.filter.NumericFilter = Ext.extend(Ext.grid.filter.Filter, {
    ...
    serialize: function(){
        ...
        for (var key in values) {
            args.push({
                type: 'numeric',
                comparison: key,
                value: this.money ? values[key] * 10000 * 100 : (this.influenceFactor ? values[key] * 1000 : values[key])
            });
        }
    ...
    validateRecord: function(record){
        var val = record.get(this.dataIndex), values = this.menu.getValue();
